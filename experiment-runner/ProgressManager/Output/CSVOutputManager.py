...
class CSVOutputManager(BaseOutputManager):
        ...
        try:
            with open(self._experiment_path / 'run_table.csv', 'w', newline='') as myfile:
                writer = csv.DictWriter(
                    myfile, fieldnames=list(run_table[0].keys()))
                writer.writeheader()
    ...
    # def shuffle_experiment_run_tablee(self):
    #     print('fhberiwybgukyehb')
    #     # Read the CSV file
    #     csv_file_path = self._experiment_path / 'run_table.csv'
    ...
    #     shuffled_df.to_csv(self._experiment_path /
    #                        'shuffled_run_table.csv', index=False)

    def shuffle_experiment_run_table(self, run_table: List[Dict]):
        run_table = random.shuffle(run_table)
    ...
    def update_row_data(self, updated_row: dict):
        ...
        with open(self._experiment_path / 'run_table.csv', 'r') as csvfile, tempfile:
            reader = csv.DictReader(
                csvfile, fieldnames=list(updated_row.keys()))
            writer = csv.DictWriter(
                tempfile, fieldnames=list(updated_row.keys()))
        ...
        output.console_log_WARNING(
            f"CSVManager: Updated row {updated_row['__run_id']}")
